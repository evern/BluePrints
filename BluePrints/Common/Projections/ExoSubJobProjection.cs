...
namespace BluePrints.Common.Projections
{
    ...
    public static class ExoMethods
    {
        public static bool CommitLineSubJob(ExoSubJobProjection projection, bool editLineAfterCommit, IDialogService BulkColumnEditDialogService, JOBCOST_HDR masterJob, string projectNumber, IPrimeroEntitiesUnitOfWork primeroUnitOfWork)
        {
            ...
            if(subjob == null)
            {
                ...
                JOB_CATEGORIES findJOB_CATEGORIES = getJobCategories(primeroUnitOfWork, projection.SubJobCode);
                if (findJOB_CATEGORIES != null)
                    subjob.CATEGORY = findJOB_CATEGORIES.CATNO;
                else
                    subjob.CATEGORY = masterJob.CATEGORY;

                projection.Category = subjob.CATEGORY;
                primeroUnitOfWork.JOBCOST_HDR.Add(subjob);
                primeroUnitOfWork.SaveChanges();
            }
            else if(editLineAfterCommit)
            {
                //edit job category
                subjob.CATEGORY = projection.Category;
            }
            ...
            if (editLineAfterCommit)
            {
                if (projection.LineId != null)
                {
                    ...
                    if (line != null)
                    {
                        ...
                        line.DESCRIPTION = projection.SubJobTitle;
                        primeroUnitOfWork.SaveChanges();
                        return true;
                    }
                    else
        ...
        private static JOB_CATEGORIES getJobCategories(IPrimeroEntitiesUnitOfWork primeroUnitOfWork, string subJobCode)
        {
            if (subJobCode.ToUpper().Contains("-" + BluePrintsResources.ProcurementPhaseCode))
                return primeroUnitOfWork.JOB_CATEGORIES.FirstOrDefault(x => x.SHORTCODE == BluePrintsResources.ProcurementPhaseCode);
            else if (subJobCode.ToUpper().Contains("-" + BluePrintsResources.DesignPhaseCode))
                return primeroUnitOfWork.JOB_CATEGORIES.FirstOrDefault(x => x.SHORTCODE == BluePrintsResources.DesignPhaseCode);
            else if (subJobCode.ToUpper().Contains("-" + BluePrintsResources.DirectPhaseCode))
                return primeroUnitOfWork.JOB_CATEGORIES.FirstOrDefault(x => x.SHORTCODE == BluePrintsResources.DirectPhaseCode);
            else if (subJobCode.ToUpper().Contains("-" + BluePrintsResources.IndirectPhaseCode))
                return primeroUnitOfWork.JOB_CATEGORIES.FirstOrDefault(x => x.SHORTCODE == BluePrintsResources.IndirectPhaseCode);
        ...
        public static IEnumerable<ExoSubJobProjection> CommitToExo(IEnumerable<ExoSubJobProjection> projections, IMessageBoxService MessageBoxService, JOBCOST_HDR masterJob, JOBCOST_LINES copyLine, PROJECT loadPROJECT, IEnumerable<USER> USERCollection, IPrimeroEntitiesUnitOfWork localPrimeroUnitOfWork, IBluePrintsEntitiesUnitOfWork bluePrintsEntitiesUnitOfWork, IDialogService BulkColumnEditDialogService, out List<ErrorMessage> errorMessages, bool updateBudgetIfExist = false, bool ignoreCostGroupCostType = false, bool validateDesignDirectIndirect = false)
        {
            ...
            foreach (ExoSubJobProjection projection in projections)
            {
                ...
                else if (projection.SubJobCode.Length > 5 && !Regex.IsMatch(projection.SubJobCode, loadPROJECT.NUMBER + BluePrintsResources.Regex_SUBJOB))
                {
        ...
        public static void ViewUpdateCategory(ExoSubJobProjection projection, IEnumerable<ExoSubJobProjection> projections)
        {
            foreach (ExoSubJobProjection relatedProjection in projections.Where(x => x.SubJobCode == projection.SubJobCode))
            {
                relatedProjection.Category = projection.Category;
                relatedProjection.Update();
            }
        }
        ...
        {
            return UpdateJOBCOST_LINES_AUDIT(bluePrintsEntitiesUnitOfWork, exoLine.SubJobCode, exoLine.DisciplineCode, exoLine.CommodityCode, exoLine.StockCode, exoLine.VariationCode, exoLine.ExoBudget, line, isDelete);
        }

        public static JOBCOST_LINES_AUDIT UpdateJOBCOST_LINES_AUDIT(IBluePrintsEntitiesUnitOfWork bluePrintsEntitiesUnitOfWork, string SubJobCode, string DisciplineCode, string CommodityCode, string StockCode, string VariationCode, decimal ExoBudget, JOBCOST_LINES line, bool isDelete = false)
        {
            JOBCOST_LINES_AUDIT jobCostLinesAudit = FindExistingOrAddJOBCOST_LINES_AUDIT(bluePrintsEntitiesUnitOfWork, line.SEQNO);
            jobCostLinesAudit.JOBCODE = DataUtils.NormalizeString(SubJobCode);
            jobCostLinesAudit.DISCIPLINE_CODE = DataUtils.NormalizeString(DisciplineCode);
            jobCostLinesAudit.COMMODITY_CODE = DataUtils.NormalizeString(CommodityCode);
            jobCostLinesAudit.STOCK_CODE = DataUtils.NormalizeString(StockCode);
            jobCostLinesAudit.VARIATION_CODE = DataUtils.NormalizeString(VariationCode);
            jobCostLinesAudit.BUDGET_FROM = line.ACTUAL_UNITCOST == null ? 0 : Convert.ToDecimal((double)line.ACTUAL_UNITCOST);
            jobCostLinesAudit.BUDGET_TO = ExoBudget;
        ...
        public static void RemoveStockItem(IPrimeroEntitiesUnitOfWork pUnitOfWork, ExoResourceProjection projection)
    {
        ...
        private static JOBCOST_RESOURCE createNewResource(IPrimeroEntitiesUnitOfWork pUnitOfWork, int staffId, string name, string title, string defaultStockCode, string shortCode)
        {
            ...
            BluePrintsDataUtils.PopulateNewJobcostResourcesDefaults(newJOBCOST_RESOURCE);
        ...
        private static STOCK_ITEMS createNewStockItem(string shortCode, string description, double? sellPrice, int? salesGLCode, int? purchGLCode, int? cosGLCode, double? stdCost, int costGroup, int costType, string department)
        {
            ...
            BluePrintsDataUtils.PopulateNewStockItemsDefaults(newSTOCK_ITEM);
       ...
       IPrimeroEntitiesUnitOfWork primeroUnitOfWork, Data.PROJECT PROJECT)
        {
            ...
            {
                exoSubJobs.Add(exoLine.ToExoSubJobProjection());
            }
        ...
        public static ExoSubJobProjection GetExoSubJobProjection(IPrimeroEntitiesUnitOfWork primeroUnitOfWork, string projectNumber, string subJobCode, string disciplineCode, string commodityCode, string variationCode)
        {
            ExoTimeAuthorisation exoLines = GetProjectLine(primeroUnitOfWork, projectNumber, subJobCode, disciplineCode, commodityCode, variationCode);
            if (exoLines == null)
                return null;

            return exoLines.ToExoSubJobProjection();
        }
            ...
            IPrimeroEntitiesUnitOfWork primeroUnitOfWork, Data.PROJECT PROJECT, IEnumerable<COMMODITY_CODE> COMMODITY_CODECollection, IEnumerable<STOCK_ITEMS> STOCK_ITEMSCollection, IEnumerable<STAFF> ExoSTAFFS, string officeName, bool ignoreValidationError = false)
        {
            ...
            foreach (ExoTimeAuthorisation exoLine in exoLines)
            {
                ...
                projection.Category = exoLine.Category;
        ...
        public static IQueryable<ExoSubJobProjection> GetExoConstructionSubJobProjection(
            ...
            IPrimeroEntitiesUnitOfWork primeroUnitOfWork, IEnumerable<COMMODITY_CODE> COMMODITY_CODECollection, IEnumerable<STAFF> ExoSTAFFS)
        {
            List<ExoTimeAuthorisation> exoAuthorisations = GetExoLinesAuthorisations(primeroUnitOfWork, PROJECT.NUMBER);
            List<ESTIMATE_ITEMProgress> estimateItems = ESTIMATE_ITEMProjectionQueries.IDeliverable_Progress_Transformation(ESTIMATE_ITEMS, PROJECT, RATES, PROGRESS, PROGRESS_ITEMS, false).ToList();
        ...
        public static ExoTimeAuthorisation GetProjectLine(IPrimeroEntitiesUnitOfWork primeroUnitOfWork, string projectNumber, string subJobCode, string disciplineCode, string commodityCode, string variationCode)
        {
            var availableLines = from JOBCOST_LINES in primeroUnitOfWork.JOBCOST_LINES
                                 ...
                                 join STOCK_ITEMS in primeroUnitOfWork.STOCK_ITEMS
                                 on JOBCOST_LINES.STOCKCODE equals STOCK_ITEMS.STOCKCODE
                                 where MAINJOB.JOBCODE == projectNumber && SUBJOB.JOBCODE == subJobCode && JOB_COSTGROUPS.SHORTCODE == disciplineCode && JOB_COSTTYPES.SHORTCODE == commodityCode && JOBCOST_LINES.X_VARIATION_CODE == variationCode
                                 select new { LINEID = JOBCOST_LINES.SEQNO, MASTERJOBNO = MAINJOB.JOBNO, MASTERJOBCODE = MAINJOB.JOBCODE, SUBJOBNO = SUBJOB.JOBNO, SUBJOBTITLE = SUBJOB.TITLE, SUBJOBNAME = SUBJOB.JOBCODE, DISCIPLINE_ID = JOBCOST_LINES.COST_CENTRE2, DISCIPLINE_CODE = JOB_COSTGROUPS.SHORTCODE, DISCIPLINE_NAME = JOB_COSTGROUPS.COSTDESC, COMMODITY_ID = JOBCOST_LINES.COST_CENTRE, COMMODITY_CODE = JOB_COSTTYPES.SHORTCODE, STOCK_CODE = STOCK_ITEMS.STOCKCODE, STOCK_NAME = STOCK_ITEMS.DESCRIPTION, COMMODITY_NAME = JOB_COSTTYPES.COSTDESC, VARIATION_CODE = JOBCOST_LINES.X_VARIATION_CODE, BUDGETED_QTY = JOBCOST_LINES.QUOTE_QTY, BUDGETED_REV = JOBCOST_LINES.LINETOTAL, BUDGETED_RATE = JOBCOST_LINES.ACTUAL_UNITCOST, FORECAST_RATE = JOBCOST_LINES.QUOTE_UNITPR, SUBJOB.CATEGORY };

            if (availableLines.Count() > 0)
                return populateExoLine(availableLines.First());
        ...
        private static ExoTimeAuthorisation populateExoLine(dynamic dbTime)
        {
            ...
            exoTime.DisciplineCode = DataUtils.NormalizeString(dbTime.DISCIPLINE_CODE);
            exoTime.DisciplineName = dbTime.DISCIPLINE_NAME;
            exoTime.CommodityId = dbTime.COMMODITY_ID;
            exoTime.CommodityCode = DataUtils.NormalizeString(dbTime.COMMODITY_CODE);
            exoTime.CommodityName = dbTime.COMMODITY_NAME;
            exoTime.StockCode = dbTime.STOCK_CODE;
            exoTime.StockName = dbTime.STOCK_NAME;
            exoTime.VariationCode = DataUtils.NormalizeString(dbTime.VARIATION_CODE);
            ...
            exoTime.Category = dbTime.CATEGORY;
    ...
    public class ExoTimeAuthorisation
    {
        ...
        public ExoSubJobProjection ToExoSubJobProjection()
        {
            ExoSubJobProjection newSubJobProjection = ViewModelSource.Create(() => new ExoSubJobProjection());
            newSubJobProjection.LineId = LineSeqNo;

            newSubJobProjection.SubJobId = SubJobNo;
            newSubJobProjection.SubJobCode = SubJobCode;
            newSubJobProjection.SubJobTitle = SubJobTitle;
            newSubJobProjection.DisciplineId = DisciplineId;
            newSubJobProjection.DisciplineCode = DisciplineCode;
            newSubJobProjection.DisciplineName = DisciplineName;
            newSubJobProjection.CommodityId = CommodityId;
            newSubJobProjection.CommodityCode = CommodityCode;
            newSubJobProjection.CommodityName = CommodityName;
            newSubJobProjection.StockCode = StockCode;
            newSubJobProjection.VariationCode = VariationCode;
            newSubJobProjection.Category = Category;
            newSubJobProjection.ExoBudget = BudgetRate;
            newSubJobProjection.AuthUserIds = new List<int>();
