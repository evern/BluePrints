    ...
    public static class ProgressQueries
    {
        ...
        public static IQueryable<BASELINE_ITEMProgress> OffsiteDirectVariationItemTransformation(IQueryable<BASELINE_ITEM> BASELINE_ITEMS, PROJECT PROJECT, PROGRESS PROGRESS, IEnumerable<PROGRESS_ITEM> PROGRESS_ITEMS, BASELINE BASELINE, IEnumerable<VARIATION> VARIATIONS, VARIATION VARIATION, IEnumerable<VARIATION_ITEM> VARIATION_ITEMS, IEnumerable<RATE> RATES, IEnumerable<DOCTYPE> DOCTYPECollection = null, IEnumerable<COMMODITY_CODE> COMMODITY_CODECollection = null)
        {
            ...
            if (PROGRESS == null || VARIATION == null)
                ...
                Baseline_ItemProgresses = ProgressQueries.OffsiteDirectProgressItemTransformation(BASELINE_ITEMS, PROJECT, PROGRESS, RATES, PROGRESS_ITEMS, VARIATIONS, false, null, DeliverableInternalNumberMode.Default, false, null, null, null, null, null, null, null, null, DOCTYPECollection, COMMODITY_CODECollection);
            ...
            IEnumerable<PROGRESS_ETC> PROGRESS_ETCS = null, bool showLoadingScreen = false, bool allowPercentageOnZeroTotalUnits = false)
        {
            ...
            if (PROGRESS == null)
                ...
                baseline_item_queryable = BASELINE_ITEMProjectionQueries.IDeliverable_Rates_Transformation(BASELINE_ITEMS, RATES, DOCTYPECollection, showLoadingScreen);
            ...
            {
                foreach (BASELINE_ITEMProjection baseline_item in baseline_item_queryable)
                {
                    if (REGISTER_HOLD_REFCollection != null)
            ...
            List<VariationAdjustment> projectVariationAdjustments = null;
            if (VARIATIONS != null)
                projectVariationAdjustments = ProjectionHelpers.BuildProjectVariationAdjustments(VARIATIONS.Where(x => x.APPROVED != null).AsQueryable(), projections);
            else
            ...
            if (forceExtrapolateDate != null)
            {
                ...
                DateTime alignedExtrapolateDate = ChronologicalHelpers.GenerateAlignedDataDate(PROGRESS.PROGRESS_START.Date, (DateTime)forceExtrapolateDate, interval);
            ...
            dynamic progress_item_by_originalguid = null;
            if (PROGRESS_ITEMS != null)
                progress_item_by_originalguid = PROGRESS_ITEMS.GroupBy(x => x.GUID_ORIBASEITEM).Select(group => new { OriginalGuid = group.Key, Progresses = group.ToList() });
