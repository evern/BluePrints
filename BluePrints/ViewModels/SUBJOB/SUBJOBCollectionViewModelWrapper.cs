...
namespace BluePrints.ViewModels
{
        ...
        <SUBJOB, SUBJOBProjection, Guid, IBluePrintsEntitiesUnitOfWork>
    {
        ...
        {
            var changedSUBJOB = (SUBJOBProjection)e.Row;
            ...
            {
                Guid? phaseGuid = null;
                string newInternalName = BluePrintsDataUtils.SUBJOB_Generate_InternalNumber(changedSUBJOB.Entity.GUID_DAREA, changedSUBJOB.Entity.GUID_DSUBAREA, loadPROJECT, AREACollection, SUBAREACollection, out phaseGuid, changedSUBJOB.Entity.GUID_DPHASE, PHASECollection);
                ...
                changedSUBJOB.Entity.INTERNAL_NAME1 = newInternalName;
            }
            if(e.Column.FieldName == "Entity.GUID_DPHASE")
            {
                Guid? phaseGuid = null;
                var newInternalName = BluePrintsDataUtils.SUBJOB_Generate_InternalNumber(changedSUBJOB.Entity.GUID_DAREA, changedSUBJOB.Entity.GUID_DSUBAREA, loadPROJECT, AREACollection, SUBAREACollection, out phaseGuid, changedSUBJOB.Entity.GUID_DPHASE, PHASECollection);
            ...
            {
                changedSUBJOB.Entity.AUTOGENERATED = false;
            }

            if (e.RowHandle != DataControlBase.NewItemRowHandle)
                MainViewModel.Save(changedSUBJOB);
        }

        protected override void CellValueNewRowChanging(CellValueChangedEventArgs e)
        {
            if (e.Column.FieldName == BindableBase.GetPropertyName(() => new SUBJOBProjection().Entity) + "." + 
                BindableBase.GetPropertyName(() => new SUBJOB().GUID_DAREA))
            {
                var changingSUBJOB = (SUBJOBProjection)e.Row;
                if (e.Value != null)
                {
                    changingSUBJOB.Entity.GUID_DAREA = (Guid)e.Value;
                    //Area is required immediately for subarea selection
                    changingSUBJOB.Entity.AREA = AREACollection.FirstOrDefault(x => x.GUID == (Guid)e.Value);
                    changingSUBJOB.Update();
                }

                //SubArea must be removed immediately to nullify subarea selection
                if (changingSUBJOB.Entity.GUID_DSUBAREA != null)
                {
                    changingSUBJOB.Entity.GUID_DSUBAREA = null;
                    changingSUBJOB.Update();
                }
            }
        ...
        protected override void CellValueExistingRowChanging(CellValueChangedEventArgs e)
        {
            if (e.Column.FieldName ==
                 BindableBase.GetPropertyName(() => new SUBJOBProjection().Entity) + "." +
                 BindableBase.GetPropertyName(() => new SUBJOB().GUID_DAREA))
            {
                var changingSUBJOB = (SUBJOBProjection)e.Row;
                Guid? oldValue = changingSUBJOB.Entity.GUID_DSUBAREA;
                if (oldValue != null)
                {
                    Guid? newValue = (Guid?)null;
                    string subAreaFieldName = BindableBase.GetPropertyName(() => new SUBJOBProjection().Entity) + "." +
                    BindableBase.GetPropertyName(() => new SUBJOB().SubAreaGuid);
                    MainViewModel.EntitiesUndoRedoManager.PauseActionId();
                    MainViewModel.EntitiesUndoRedoManager.AddUndo(changingSUBJOB, subAreaFieldName, oldValue, newValue, EntityMessageType.Changed);
                    changingSUBJOB.Entity.GUID_DSUBAREA = newValue;
                    changingSUBJOB.Update();
                }
            }
        ...
        {
            if (
                e.Column.FieldName == BindableBase.GetPropertyName(() => new SUBJOBProjection().Entity) + "." + BindableBase.GetPropertyName(() => new SUBJOB().STARTDATE) 
                || 
                e.Column.FieldName == BindableBase.GetPropertyName(() => new SUBJOBProjection().Entity) + "." + BindableBase.GetPropertyName(() => new SUBJOB().ENDDATE))
            {
                DateTime startDate;
                DateTime endDate;
                ...
                var changingSUBJOB = (SUBJOBProjection)e.Row;
                if (e.Column.FieldName == BindableBase.GetPropertyName(() => new SUBJOBProjection().Entity) + "." +
                                          BindableBase.GetPropertyName(() => new SUBJOB().STARTDATE))
                {
                    startDate = (DateTime)e.Value;
                    endDate = (DateTime)changingSUBJOB.Entity.ENDDATE;
                    if (endDate < startDate)
                    {
                        endDate = BluePrintsDataUtils.SUBJOB_Calculate_EndDate(startDate, loadPROJECT);
                        changingSUBJOB.Entity.ENDDATE = endDate;
                    }
                }
                else
                {
                    endDate = (DateTime)e.Value;
                    startDate = (DateTime)changingSUBJOB.Entity.STARTDATE;
                    if (endDate < startDate)
                    {
                        startDate = BluePrintsDataUtils.SUBJOB_Calculate_StartDate(endDate, loadPROJECT);
                        changingSUBJOB.Entity.STARTDATE = startDate;
                    }
                }
                ...
                BluePrintsDataUtils.SUBJOB_Calculate_ReviewPeriod(ref reviewStartDate, ref reviewEndDate, loadPROJECT,
                    false);
                changingSUBJOB.Entity.REVIEWSTARTDATE = reviewStartDate;

                if (reviewEndDate >= endDate)
                    changingSUBJOB.Entity.REVIEWENDDATE = endDate;
                else
                    changingSUBJOB.Entity.REVIEWENDDATE = reviewEndDate;
