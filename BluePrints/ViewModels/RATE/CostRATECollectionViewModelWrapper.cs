...
namespace BluePrints.ViewModels
{
    public class CostRATECollectionViewModelWrapper : RATECollectionViewModelWrapper
    {
        ...
        protected override IQueryable<RATE> rateCommodityProjection(IRepositoryQuery<RATE> rates)
        {
            ...
            //populate charge and cost type
            foreach (var queryTransactionRate in queryTransactionRates)
            {
                PHASE findPHASE = PHASECollection.FirstOrDefault(x => queryTransactionRate.RawPhaseCode == x.INTERNAL_NUM);
                if (findPHASE != null && findPHASE.CHARGE_TYPE != null && findPHASE.PHASE_TYPE != null)
                {
                    queryTransactionRate.ChargeType = (ChargeType)findPHASE.CHARGE_TYPE;
                    queryTransactionRate.PhaseType = (PhaseType)findPHASE.PHASE_TYPE;
                }
            }
        ...
        private void setRecommendedRate(RATE rate, List<TransactionRate> groupedTransactionRates, bool isEditRateField = false)
        {
            ...
            AREA findAREA = rate.AREA;
            if (findAREA == null && rate.GUID_AREA != null)
                findAREA = AREACollection.FirstOrDefault(x => x.GUID == rate.GUID_AREA);

            AREA findSubAREA = rate.SUBAREA;
            if (findSubAREA == null && rate.GUID_SUBAREA != null)
                findSubAREA = SUBAREACollection.FirstOrDefault(x => x.GUID == rate.GUID_SUBAREA);
            ...
            string s;
            if (rate.GUID.ToString() == "2f1bad2a-e21a-42c2-9199-93a9992cffe1")
                s = string.Empty;

            IEnumerable<TransactionRate> transactionRatesByPhase = groupedTransactionRates.Where(x => findPHASE == null || (x.ChargeType == findPHASE.CHARGE_TYPE && x.PhaseType == findPHASE.PHASE_TYPE));
        ...
        {
            RATE uncommittedRATE = Entities.FirstOrDefault(x => !x.IsRateExists && x.GUID != entity.GUID && x.GUID_PHASE == entity.GUID_PHASE && x.GUID_AREA == entity.GUID_AREA && x.GUID_DISCIPLINE == entity.GUID_DISCIPLINE && x.GUID_DEPARTMENT == entity.GUID_DEPARTMENT && x.COMMODITY_CODE == entity.COMMODITY_CODE && x.VARIATION_CODE == entity.VARIATION_CODE);
        ...
        {
            if (field_name == BindableBase.GetPropertyName(() => new RATE().GUID_PHASE) || field_name == BindableBase.GetPropertyName(() => new RATE().GUID_AREA) || field_name == BindableBase.GetPropertyName(() => new RATE().GUID_DISCIPLINE))
            {
        ...
        {
            if (field_name == BindableBase.GetPropertyName(() => new RATE().GUID_AREA) || field_name == BindableBase.GetPropertyName(() => new RATE().GUID_PHASE) || field_name == BindableBase.GetPropertyName(() => new RATE().GUID_DISCIPLINE))
            {
                populatePHASE(projection);
                projection.SetLookupProperties(CombinedCommodityCodeCollection, DISCIPLINECollection, SUBAREACollection);
            }
        ...
        public override string ViewName
        {
            ...
            get { return "CostRATECollectionViewModelWrapper_v3"; }
        }
    ...
    public class TransactionRate
    {
        ...
        public string RawVariationCode { get; set; }
