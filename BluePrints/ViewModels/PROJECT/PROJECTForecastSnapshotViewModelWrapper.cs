...
namespace BluePrints.ViewModels
{
    public class PROJECTForecastSnapshotViewModelWrapper : BluePrintsEntitiesCollectionWrapper<Data.PROJECT, Data.PROJECT, Guid, IBluePrintsEntitiesUnitOfWork>
    {
        ...
        private void updateDataPointsTable()
        {
            ...
            uniqueWBSName =>
            {
                ...
                //}
                UniqueForecastJob uniqueForecastJob = new UniqueForecastJob(projectLines, subJobCode, disciplineCode, commodityCode, variationCode, FixedDataDate, PreviousDataDate, AllFORECAST_JOB_HOUR_SNAPSHOTCollection, FORECAST_COMMENTCollection, cachedFORECASTCollection, IsShowUninvoicedOnly);
        ...
        private void InitializeColumnSource(ObservableCollection<ColumnDescriptor> columns, ObservableCollection<SummaryDescriptor> summaries, List<DateTime> alignedDates, bool isChild)
        {
            ...
            if (!isChild)
            {
                ...
                columns.Add(new ColumnDescriptor() { FieldName = "Entity.ActualCosts", ReadOnly = true, Header = "Actual Costs (B)", Fixed = FixedStyle.Left, Width = 70, Settings = SettingsType.Number, Mask = "c0", HeaderToolTip = "Costs burned to Date. Materials to Data Date and Time is the Sunday before Data Date" });
                ...
                columns.Add(new ColumnDescriptor() { FieldName = "Entity.ActualCumulativeCostsPreviousDataDate", ReadOnly = true, Header = "Actual Costs Post DD", Visible = false, Fixed = FixedStyle.Left, Width = 50, Settings = SettingsType.Number, Mask = "c0", HeaderToolTip = "Actual costs post data date" });
                ...
                columns.Add(new ColumnDescriptor() { FieldName = "Entity.Outstanding", ReadOnly = true, Header = "Outstanding (C)", Fixed = FixedStyle.Left, Width = 70, Settings = SettingsType.POError, Mask = "c0", HeaderToolTip = "Open Commitment, amount left on purchase order (outstanding PO) or amount left on P6 forecasts" });
    ...
    {
        public UniqueForecastJob(IEnumerable<ExoSubJobProjection> projectLines, string subJobCode, string disciplineCode, string commodityCode, string variationCode, DateTime dataDate, DateTime previousDataDate, IEnumerable<FORECAST_JOB_HOUR_SNAPSHOT> AllFORECAST_JOB_HOURCollection, IEnumerable<FORECAST_COMMENT> FORECAST_COMMENTCollection, IEnumerable<FORECAST> FORECASTCollection, bool isShowUninvoicedOnly)
        {
            ...
            DataDateActualCollection = FORECASTCollection.Where(x => x.FORECAST_DATE != null).Where(x => x.FORECAST_DATE.Date == dataDate.Date).Where(x => x.SUBJOB_CODE == subJobCode && x.DISCIPLINE_CODE == disciplineCode && x.COMMODITY_CODE == commodityCode && x.VARIATION_CODE == variationCode).Where(x => x.FORECAST_TYPE == ForecastDataType.DataDateForecast).ToList();
        ...
        public List<FORECAST> DataDateActualCollection { get; set; }
